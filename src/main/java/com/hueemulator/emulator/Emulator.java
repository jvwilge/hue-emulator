...
public class Emulator {
    ...
    public void startServers() {
        ...
        if (!controller.getPort().equals("80")) {
            controller.addTextToConsole("UPnP works best with the Emulator running on port 80.  Apps written with the Java/iOS " +
                    "SDK's wont connect to the Emulator.", Color.RED, true);
        }
    ...
    public void stopServer() {
        if (server != null && server.getHttpServer() != null) {
            ...
            controller.addTextToConsole("Stopping the Server on port: " + server.getHttpServer().getAddress().getPort(), Color
                    .RED, true);
        }
        if (upnpServer != null) {
